...
# BEFORE_TEXT_CONTENT-NEXT:  33e80600 0000c3e8
...
# BEFORE_ALIGNMENT-NEXT: LOAD           0x000120 0x0000000000201120 0x0000000000201120 0x000011 0x000011 R E 0x1000
...
# BEFORE_SYM: {{[0-9a-f]+}} t _foo3
# BEFORE_SYM-NEXT: {{[0-9a-f]+}} T _start
# BEFORE_SYM-NEXT: {{[0-9a-f]+}} t _foo2
# BEFORE_SYM-NEXT: {{[0-9a-f]+}} t _foo1

# RUN: ld.lld --enable-huge-text %t.o -o %t_cg_profile2.out --gc-sections
...
# AFTER_TEXT_CONTENT-NEXT:  33e80600 0000c3e8
...
# AFTER_ALIGNMENT-NEXT: LOAD           0x200000 0x0000000000400000 0x0000000000400000 0x000011 0x000011 R E 0x200000
...
# AFTER_SYM: {{[0-9a-f]+}} T __hot_start
# AFTER_SYM-NEXT: {{[0-9a-f]+}} t _foo3
# AFTER_SYM-NEXT: {{[0-9a-f]+}} T _start
# AFTER_SYM-NEXT: {{[0-9a-f]+}} t _foo2
# AFTER_SYM-NEXT: {{[0-9a-f]+}} T __hot_end
# AFTER_SYM-NEXT: {{[0-9a-f]+}} t _foo1
