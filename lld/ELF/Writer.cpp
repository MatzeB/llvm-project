...
template <class ELFT> void Writer<ELFT>::reduceNonLineDwarfDebug() {
  ...
  AbbrSec->writeTo<ELFT>(AbbrBuf.data());
  std::vector<uint8_t> InfoBuf(InfoSec->size);
  InfoSec->writeTo<ELFT>(InfoBuf.data());
  ...
  ArangesSec->writeTo<ELFT>(reinterpret_cast<uint8_t *>(&ReducedAranges[0]));
  {
...
static DenseMap<const InputSectionBase *, int> buildSectionOrder() {
  ...
  if (config->enableHugeText) {
    ...
    // __hot_end to -1 < 0
    priority -= 1;
    symbolOrder.insert({"__hot_start", {priority - 1, false}});
    symbolOrder.insert({"__hot_end", {-1, false}});
  }
...
template <class ELFT> void Writer<ELFT>::reorderSectionByRelocationAddend() {
  ...
  for (BaseCommand *base : script->sectionCommands) {
    auto *sec = dyn_cast<OutputSection>(base);
    if (!sec || sec->name != ".text" || sec->sectionCommands.empty())
      continue;
    for (BaseCommand *b : sec->sectionCommands) {
