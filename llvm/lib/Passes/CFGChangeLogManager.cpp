...
// (Machine)BasicBlock.
struct Node {
  Node(const void *Ptr, const std::string &Name, std::optional<uint64_t> ProfCount,
...
template <> void CFG::populateCFG<Function>(const Function *F) {
  ...
  for (auto BI = F->begin(); BI != F->end(); ++BI) {
    ...
    for (auto SI = succ_begin(BB); SI != succ_end(BB); ++SI, ++succIndex) {
      ...
      double Percent =
          std::rint(((double)Prob.getNumerator() / Prob.getDenominator()) * 100.0 *
               100.0) /
          100.0;
...
template <> void CFG::populateCFG<MachineFunction>(const MachineFunction *MF) {
  ...
  for (auto BI = MF->begin(); BI != MF->end(); ++BI) {
    ...
    for (auto SI = MBB->succ_begin(); SI != MBB->succ_end(); ++SI) {
      ...
      double Percent =
          std::rint(((double)N / Prob.getDenominator()) * 100.0 * 100.0) / 100.0;
...
void CFGChangeLogManager::runAfterPass(StringRef PassID, Any IR) {
  if (PassID.starts_with("PassManager<") || PassID.contains("PassAdaptor<"))
    return;
  ...
  } else if (any_cast<const Function *>(&IR)) {
