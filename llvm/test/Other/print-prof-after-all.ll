...
; RUN: opt < %s -enable-new-pm=0 -pgo-instr-use -pgo-test-profile-file=%t.profdata -print-after-all -print-prof=all -S 2>&1 | FileCheck %s --check-prefix=ALL_INSTR
; RUN: opt < %s -enable-new-pm=0 -pgo-instr-use -pgo-test-profile-file=%t.profdata -print-after-all -print-prof=names -S 2>&1 | FileCheck %s --check-prefix=NAMES_INSTR
; RUN: opt < %s -enable-new-pm=0 -sample-profile -sample-profile-file=%S/Inputs/print-prof-after-all.prof -print-after-all -print-prof=all -S 2>&1 | FileCheck %s --check-prefix=ALL_SAMPLE
; RUN: opt < %s -enable-new-pm=0 -sample-profile -sample-profile-file=%S/Inputs/print-prof-after-all.prof -print-after-all -print-prof=names -S 2>&1 | FileCheck %s --check-prefix=NAMES_SAMPLE

target datalayout = "e-m:e-i64:64-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

; ALL_INSTR: IR Dump After
; NAMES_INSTR: IR Dump After
; ALL_SAMPLE: IR Dump After
; NAMES_SAMPLE: IR Dump After
