; facebook T53546053
; RUN: opt < %s -enable-new-pm=0 -verify -loop-simplify -S -cfg-change-log-funcs=test -o /dev/null 2>&1 | FileCheck %s --check-prefix=LegacyPM
; RUN: opt < %s -passes='verify,function(loop-simplify)' -S -cfg-change-log-funcs=test -o /dev/null 2>&1 | FileCheck %s --check-prefix=NewPM

; LegacyPM: CFGChangeLog After Canonicalize natural loops (function: test)
; LegacyPM: CFGChangeLog: changed %be[[ID1:[0-9]]]
; LegacyPM-NEXT: -> %be[[ID1]]{{.+}}, outgoings 0x[[TARGET:[0-9a-f]+]]:
; LegacyPM-NEXT: CFGChangeLog: changed %be[[ID2:[0-9]]]
; LegacyPM-NEXT: -> %be[[ID2]]{{.+}}, outgoings 0x[[TARGET]]:
; LegacyPM-NEXT: CFGChangeLog: inserted %loop.backedge(0x[[TARGET]], prof N/A)
