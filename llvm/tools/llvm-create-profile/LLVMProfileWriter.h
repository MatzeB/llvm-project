...
public:
  explicit LLVMProfileBuilder(const StringIndexMap &name_table)
      : profiles_(), result_(llvm::sampleprof_error::success), inline_stack_(),
  ...
  const SampleProfileMap &ConvertProfiles(const SymbolMap &symbol_map);

  const SampleProfileMap &GetProfiles() const { return profiles_;
  }
