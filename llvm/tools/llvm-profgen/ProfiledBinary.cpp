...
void ProfiledBinary::decodePseudoProbe(const ELFObjectFileBase *Obj) {
  ...
  if (ShowDisassemblyOnly) {
    ...
    for (auto *F : ProfiledFunctions) {
      ...
      for (auto I = AltGUIDs.first; I != AltGUIDs.second; ++I)
        GuidFilter.insert(I->second);
  ...
  for (const SymbolRef &Symbol : Obj->symbols()) {
    ...
    if (!Range) {
      ...
      if (Ret.second) {
        ...
        HashBinaryFunctions[Function::getGUID(SymName)] =
            &Func;
      }
      ...
      AlternativeFunctionGUIDs.emplace(
          Range->Func, Function::getGUID(SymName));
  ...
  for (auto *Func : ProfiledFunctions) {
    ...
    for (auto &[StartAddr, EndAddr] : Func->Ranges) {
      ...
      if (!Range->IsFuncEntry)
        continue;
      const auto &Probe = Address2ProbesMap.find(StartAddr);
      if (Probe != Address2ProbesMap.end() && EndAddr <= std::next(Probe)->first) {
        MCDecodedPseudoProbeInlineTree *InlineTreeNode =
            Probe->second.begin()->getInlineTreeNode();
