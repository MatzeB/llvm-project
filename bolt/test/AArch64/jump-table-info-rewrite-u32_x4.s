## Check AArch64 .llvm_jump_table_info U32_X4 recognition/rewriting path.
##
## Format 7 encodes entries as 4-byte unsigned values with an implicit shift:
##   target = base + (entry << 2)
##
## This test verifies BOLT parses the section and keeps a valid rewritten
## format-7 table in output.
...
# RODATA-NEXT: 0x{{[0-9a-f]+}} 0c000000 09000000 05000000
...
.Ldefault:
  ...
  .p2align 2
.LJTI0_0:
  .word ((.Lcase0 - _start) >> 2)
  .word ((.Lcase1 - _start) >> 2)
  .word ((.Lcase2 - _start) >> 2)

  .section .llvm_jump_table_info,"o",@0x6fff4c0e,_start
  .byte 7                               // format 7: 4-byte unsigned, shr 2
