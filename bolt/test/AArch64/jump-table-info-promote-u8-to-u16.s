## Check that an AArch64 format-2 (U8_X4) jump table is promoted to
## format-3 (U16_X4) when block reordering makes U8 entries overflow.

# REQUIRES: system-linux

# RUN: %clang %cflags -fuse-ld=lld %s -o %t.exe -Wl,-q
# RUN: llvm-bolt %t.exe -o %t.bolt --reorder-blocks=reverse --jump-tables=move -v=1 2>&1 \
# RUN:   | FileCheck %s --check-prefix=LOG
# RUN: llvm-objdump -d --no-show-raw-insn %t.bolt | FileCheck %s --check-prefix=DIS

# LOG: BOLT-INFO: updated AArch64 jump table {{.*}} to AARCH64_U16_X4
# LOG: BOLT-INFO: promoted 1 AArch64 jump table(s)

# DIS: <_start>:
# DIS: ldrh	w11, [x9, w8, uxtw #1]
